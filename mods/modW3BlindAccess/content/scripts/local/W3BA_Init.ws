...
// Use an array as a holder to work around const field assignment.
// The array reference is const, but array contents can be modified.
@addField(CR4Player)
var w3ba_coreManagerHolder : array<W3BA_CoreManager>;

function W3BA_GetCoreManager() : W3BA_CoreManager
{
    if (!thePlayer) { return NULL; }
    if (thePlayer.w3ba_coreManagerHolder.Size() == 0) { return NULL; }
    return thePlayer.w3ba_coreManagerHolder[0];
}
...
function W3BA_EnsureInitialized()
{
    ...
    if (thePlayer.w3ba_coreManagerHolder.Size() == 0)
    {
        core = new W3BA_CoreManager in thePlayer;
        core.Initialize();
        thePlayer.w3ba_coreManagerHolder.PushBack(core);
    }
}
